...
<div class="max-w-full mx-auto px-4">
    <div class="mb-6">
        <div class="flex justify-between items-center">
            <div>
                <h1 class="text-3xl font-bold text-gray-900">Penilaian Bulk AHP</h1>
                <p class="mt-2 text-sm text-gray-600">Masukkan nilai untuk semua nasabah dan sub kriteria sekaligus. Gunakan skala 1-9. Klik nama nasabah untuk melihat parameter.</p>
            </div>
    ...
    <div class="flex gap-6" id="main-container">
        ...
        <div class="transition-all duration-300" id="main-content">
            <form action="{{ route('evaluations.bulk.store') }}" method="POST">
                @csrf
                            ...
                            <thead class="bg-gray-50">
                                <tr>
                                    ...
                                    @foreach($subcriteria as $sub)
                                        <th class="px-2 py-3 text-center text-xs font-medium text-gray-500 uppercase tracking-wider border-l min-w-20">
                                            <div class="text-xs">{{ $sub->criteria->name }}</div>
                                            <div class="font-bold">{{ $sub->code }}</div>
                                            <div class="text-xs break-words">{{ $sub->name }}</div>
                                        </th>
                                    @endforeach
                                </tr>
                            </thead>
                                ...
                                @foreach($alternatives as $alternative)
                                    <tr class="hover:bg-gray-50">
                                        <td class="px-4 py-3 whitespace-nowrap font-medium text-gray-900 sticky left-0 bg-white z-10 border-r min-w-48">
                                            <button type="button" onclick="showParameters({{ $alternative->id }}, '{{ $alternative->name }}')" 
                                        ...
                                        @foreach($subcriteria as $sub)
                                            @php
                                                $key = $alternative->id . '_' . $sub->id;
                                                $currentValue = $evaluations->get($key)?->value ?? '';
                                            @endphp
                                            <td class="px-2 py-3 text-center border-l">
                                                <input type="number" 
                                                       name="evaluations[{{ $alternative->id }}][{{ $sub->id }}]"
                                                       value="{{ $currentValue }}"
                                                       min="1" max="9" step="1"
                                                       class="w-16 text-center border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 text-sm">
                                            </td>
                                        @endforeach
                                    </tr>
                                @endforeach
