...
<div class="max-w-2xl mx-auto px-4">
    <div class="mb-6">
            ...
            <ol class="inline-flex items-center space-x-1 md:space-x-3">
                ...
                <li>
                    <div class="flex items-center">
                        ...
                        <a href="{{ route('subcriteria.show', $subcriteria) }}" class="ml-1 text-sm font-medium text-gray-500 hover:text-blue-600 md:ml-2">{{ $subcriteria->name }}</a>
                    </div>
                </li>
                <li>
                    <div class="flex items-center">
        ...
        <h1 class="text-3xl font-bold text-gray-900">Edit Sub Kriteria</h1>
        <p class="mt-2 text-sm text-gray-600">Perbarui informasi sub kriteria {{ $subcriteria->code }}</p>
    </div>

    <div class="bg-white shadow-lg rounded-lg overflow-hidden">
        <form action="{{ route('subcriteria.update', $subcriteria) }}" method="POST" class="p-6">
            @csrf
            @method('PUT')
            ...
            <div class="space-y-6">
                    ...
                    <select name="criteria_id" id="criteria_id" required
                        ...
                        @foreach($criteria as $criterion)
                            <option value="{{ $criterion->id }}" 
                                    {{ (old('criteria_id', $subcriteria->criteria_id) == $criterion->id) ? 'selected' : '' }}>
                ...
                <div>
                    ...
                    <input type="text" name="code" id="code" required
                           ...
                           value="{{ old('code', $subcriteria->code) }}" placeholder="Contoh: C1.1, C1.2, etc."
                           maxlength="10">
                    @error('code')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                ...
                <div>
                    ...
                    <input type="text" name="name" id="name" required
                           ...
                           value="{{ old('name', $subcriteria->name) }}" placeholder="Contoh: Gaji Pokok, Tunjangan, dll."
                           maxlength="255">
                    @error('name')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                ...
                <div>
                    ...
                    <textarea name="description" id="description" rows="4"
                              ...
                              placeholder="Deskripsi detail tentang sub kriteria ini...">{{ old('description', $subcriteria->description) }}</textarea>
                    @error('description')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                ...
                @if($subcriteria->weight > 0)
                    <div class="bg-green-50 border border-green-200 rounded-lg p-4">
                        <h4 class="text-sm font-medium text-green-900 mb-2">Informasi Bobot</h4>
                        <p class="text-sm text-green-800">
                            Bobot saat ini: <strong>{{ number_format($subcriteria->weight, 4) }}</strong>
                        </p>
    ...
    <div class="mt-8 bg-white shadow-lg rounded-lg overflow-hidden border-l-4 border-red-400">
        <div class="p-6">
            ...
            <p class="text-sm text-gray-600 mb-4">
                Menghapus sub kriteria akan menghapus semua data evaluasi terkait. Tindakan ini tidak dapat dibatalkan.
            </p>
            <form action="{{ route('subcriteria.destroy', $subcriteria) }}" method="POST" class="inline"
                  onsubmit="return confirm('Apakah Anda yakin ingin menghapus sub kriteria ini? Semua data evaluasi terkait akan ikut terhapus.')">
                @csrf
                @method('DELETE')
                <button type="submit" 
